...
class RouteCollector
{
    ...
    /**
     * 自动生成RESTful风格前缀
     * UserController -> /api/users (复数)
     * UserDetailController -> /api/user-details
     * Api\V1\UserController -> /api/v1/users
     */
    private function generateRestfulPrefix(string $className): string
    {
        ...
        $resourceName = $this->camelToKebab($resourceName);
    ...
    private function getRoutePath(string $methodName, string $httpMethod, Mapping $routeAnnotation): string
    {
        ...
        // 检查是否符合RESTful映射
        if (isset($this->restfulMapping[$methodName])) {
            [$expectedMethod, $pathTemplate] = $this->restfulMapping[$methodName];
            
            // 如果HTTP方法匹配，使用RESTful路径模板
        ...
        // 检查是否符合资源子操作映射
        if (isset($this->resourceActionMapping[$methodName])) {
            [$expectedMethod, $pathTemplate] = $this->resourceActionMapping[$methodName];
            
            // 如果HTTP方法匹配，使用资源子操作路径模板
            if (strtoupper($httpMethod) === $expectedMethod) {
                return $pathTemplate;
            }
        }
        ...
        // 默认：方法名转路径（驼峰转中划线）
        return '/' . $this->camelToKebab($methodName);
    }
    ...
    private function generateSmartPath(string $methodName, ReflectionMethod $method): string
    {
        ...
        // 转换方法名
        $pathSegment = $this->camelToKebab($methodName);
    ...
    {
        return isset($this->restfulMapping[$methodName]) || isset($this->resourceActionMapping[$methodName]);
    }
